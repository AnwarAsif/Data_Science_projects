...
def smallest_clauses(clauses):
    tiny_clause_size = 1000
    for clause in clauses:
        if (len(clause) < tiny_clause_size):
            #if len(clause) > 1:
            tiny_clause_size =  len(clause)
    tiny_clauses = [clause for clause in clauses if len(clause) == tiny_clause_size]
    return tiny_clauses
            
def MOMS(clauses):
    try:        
        return most_frequent(smallest_clauses(clauses))  
    except Exception as inst:
        #print(type(inst))
        variable_selection(clauses)
 
def most_frequent(clauses):
       literals_count = find_literals(clauses)
       
       most_fq = max(literals_count.items(), key=operator.itemgetter(1))[0]
       return most_fq 
     
def dpll_moms(clauses, givenliteral):
