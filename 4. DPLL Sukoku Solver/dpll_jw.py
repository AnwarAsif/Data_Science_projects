...
def jw(clauses):
    ...
    for clause in clauses:
        for literal in clause: 
            if literal in jw:        
                jw[literal] += math.pow(2, -len(clause))
            else: 
                jw[literal] = math.pow(2, -len(clause))
    highest_jw = max(jw.items(), key=operator.itemgetter(1))[0]
...
def dpll_jw(clauses, givenliteral):
    ...
    solution = dpll_jw(new_clauses, new_literals)
